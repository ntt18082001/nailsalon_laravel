<x-admin-layout title="Danh sách ticket">
                ...
                <tr>
                    ...
                    <td >
                        ...
                        @elseif ($item->status_id == 2)
                            <span class="badge border bg-info status-{{$item->id}}">{{ $item->status->name }}</span>
                        @elseif ($item->status_id == 3)
                            <span class="badge border bg-success status-{{$item->id}}">{{ $item->status->name }}</span>
                        @elseif ($item->status_id == 4)
                            <span class="badge border bg-dark status-{{$item->id}}">{{ $item->status->name }}</span>
                        @endif
                    </td>
                    <td>{{ date('d-m-Y H:i:s', $item->start_at / 1000) }}</td>
                    <td>{{ date('d-m-Y H:i:s', $item->end_at / 1000) }}</td>
                    <td class="fit">
                        <a href="{{ route('admin.ticket.detail', ['id' => $item->id]) }}" class="btn btn-info">
                            Xem
                        </a>
                        <button type="button" data-order-id={{ $item->id }} data-status-id={{ $item->status_id }}
                            class="btn btn-primary btn-update-status btn-status{{$item->id}}" data-bs-toggle="modal"
    ...
    <x-slot name="header">
        <link rel="stylesheet" href="{{ asset('noti-aweasome/css/style.css') }}"></link>
